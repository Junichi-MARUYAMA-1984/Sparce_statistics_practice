...
warm_start <- function(X, y, lambda_max = 100, alpha = 1) {
  ...
  coef_seq[1, ] <- elastic_net(X, y, lambda = lambda_seq[1], alpha = alpha)$beta # 最大lambdaの時のbetaを計算。以降の計算の初期値となる。
  
  for (k in 2:r) {
    # 一つ前のlambda値で求めたbeta値を用いて、次のlambda値でのbetaを計算する。
    coef_seq[k, ] <- elastic_net(X, y, lambda = lambda_seq[k], beta = coef_seq[(k - 1), ], alpha = alpha)$beta
  }
...
coef_seq <- warm_start(X, y, 200, alpha = 0)
...
for (j in 1:p) {
  lines(lambda_seq, base = 10, coef_seq[, j], col = j)
}
